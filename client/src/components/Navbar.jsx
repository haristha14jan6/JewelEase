...
const Navbar = () => {
    ...
    const location = useLocation() //we use this location to identify the path
...
${location.pathname === "/" ? "bg-light" : "bg-white"}`}>
        <Link to='/'>
        <motion.img whileHover={{scale:1.05}} src={assets.logo1} alt="logo" className="h-8"/>
        </Link>
...
duration-300 z-50 ${location.pathname === "/" ?"bg-light": "bg-white"} ${open? "max-sm:translate-x-0":"max-sm:translate-x-full"}`}>
            ...
            <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
                ...
                className='cursor-pointer'>{isOwner?'Dashboard':'List jewels'}</button> 
